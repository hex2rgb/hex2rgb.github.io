(() => {
  // duration.js
  $(function() {
    var canvas = document.getElementById("duration-day");
    canvas.width = $("#duration-day").width();
    ...
    function Particle(x, y) {
      ...
      this.futurRadius = randomInt(1, 1.1);
      this.radius = 1.1;
      this.dying = false;
      this.base = [x, y];
    ...
    function Shape(text, size, x, y) {
      ...
      this.y = y;
      this.placement = [];
      function getValue(that2) {
        context.textAlign = "center";
        context.font = that2.size + "px arial";
        context.fillText(that2.text, that2.x, that2.y);
    ...
    function duration(moment2) {
      var startDate = moment2("2020-08-20 00:00:00");
      var endDate = moment2();
      var diffTime = moment2.duration(endDate.format("x") - startDate.format("x"));
      var day = diffTime.days();
      var hours = diffTime.hours();
      var minutes = diffTime.minutes();
      var second = diffTime.seconds();
      return "运行" + day + "天" + hours + "小时" + minutes + "分" + second + "秒";
    }
    function drawFrame() {
      context.clearRect(0, 0, W, H);
      var word = new Shape(duration(moment), 60, W / 2, H / 2);
